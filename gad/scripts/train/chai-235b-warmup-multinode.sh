...
# [CRITICAL] Connect to the Ray Cluster we just built
export RAY_ADDRESS="auto"

# Preserve your argument parsing logic
while [[ $# -gt 0 ]]; do
    case $1 in
...
# [CHANGE] Doubled Batch Size (16) because we have 2x GPUs
TRAIN_BATCH_SIZE=16
VAL_BATCH_SIZE=16
MINI_BATCH_SIZE=8
...
# [CHANGE] Multi-Node Config
NNODES=2
GPUS_PER_NODE=8

# Safety Check
if [ ! -d "$MODEL_PATH" ]; then
    echo "🚨 CRITICAL ERROR: Model path $MODEL_PATH not found."
    echo "Ensure the model is mounted on THIS pod (and the worker pod!)."
    exit 1
fi
