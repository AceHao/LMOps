...
def extract_query_from_conversation(content) -> Optional[str]:
...
def run_evaluation(
    ...
    if skipped_missing_response > 0:
        print(f"  Skipped {skipped_missing_response} pairs due to missing responses")
...
def print_summary(stats: Dict, output_file: str):
    ...
    print(f"Invalid judgments: {stats['invalid']} ({stats['invalid']/stats['total']*100:.1f}%)" if stats['total'] > 0 else "Invalid judgments: N/A")
    print("-"*60)
